...
const KeyboardAwareScrollable = ({
  ...
  const commonProps = {
    ...
    keyboardShouldPersistTaps: (childAttrs['keyboardShouldPersistTaps'] ?? 'handled') as 'handled' | 'always' | 'never',
    keyboardDismissMode: (childAttrs['keyboardDismissMode'] ?? 'on-drag') as 'none' | 'interactive' | 'on-drag',
...
const RawScreen = ({
  ...
  // Uniwind 将 style 转为 [tailwindObj, originalObj] 格式，需解构还原
  const rawStyle = Array.isArray(style) ? (style[1] || style[0]) : style;
  const scrollViewStyle = useScrollContainer && rawStyle ? (() => {
    const { justifyContent, alignItems, ...rest } = rawStyle as any;
    return rest;
  })() : rawStyle;
  const contentAlignStyle = useScrollContainer && rawStyle ? (() => {
    const { justifyContent, alignItems } = rawStyle as any;
    return (justifyContent !== undefined || alignItems !== undefined) ? { justifyContent, alignItems } : {};
  })() : {};
  ...
  return (
    ...
    <View style={wrapperStyle}>
      ...
      {useScrollContainer ? (
        ...
        <Container style={[styles.innerContainer, scrollViewStyle]} {...containerProps}>
          {children}
        </Container>
      ) : (
