...
export default function PreviewScreen() {
  ...
  // 导出图纸（高分辨率 PNG，无损放大清晰）
  const handleExport = async () => {
    if (!imageId) return;
    setIsExporting(true);
    try {
      const pngUrl = `${API_BASE}/api/v1/bead/export-png/${imageId}`;

      if (Platform.OS === 'web') {
        // Web: 下载高分辨率 PNG（含编号像素 + 色号图例）
        webDownload(pngUrl, `bead-pattern-${imageId}.png`);
      } else {
        // 原生端：下载 PNG
        const cacheDir = (FileSystem as any).cacheDirectory || '';
        const pngFileUri = `${cacheDir}bead-pattern-${imageId}.png`;
        const download = await FileSystem.downloadAsync(pngUrl, pngFileUri);
        if (download.status === 200) {
          const isAvailable = await Sharing.isAvailableAsync();
          if (isAvailable) {
            await Sharing.shareAsync(pngFileUri, {
              mimeType: 'image/png',
              dialogTitle: '保存拼豆图纸',
            });
          } else {
  ...
  const handleExportLegend = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        webDownload(legendUrl, `bead-legend-${imageId}.png`);
        Alert.alert('成功', '图例已开始下载');
      } else {
        const cacheDir = (FileSystem as any).cacheDirectory || '';
