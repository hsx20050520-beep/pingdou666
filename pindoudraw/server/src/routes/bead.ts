...
import { 
  ...
  generatePatternPNG, 
  generateLegendPNG,
  generateCombinedExportPNG,
  getProcessedImage 
} from '../image-processor';
import { generatePatternPDF } from '../pdf-export';
import { generatePatternPNG_v2 } from '../png-export';
...
router.post('/process', upload.single('image'), async (req: Request, res: Response) => {
  try {
    ...
    const paletteCompression = (body.paletteCompression as string) || 'off';
    const saturation = 1.0; // fixed at 1.0 to prevent color drift
    const useDithering = body.useDithering === 'true' || body.useDithering === true;
...
router.get("/export-pdf/:id", async (req: Request, res: Response) => {
  try {
    ...
    const pdfBuffer = await generatePatternPDF(image, 16);

    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `attachment; filename="bead-pattern-${id}.pdf"`);
    res.send(pdfBuffer);
  } catch (error: any) {
    console.error("Export PDF error:", error);
    res.status(500).json({ error: "PDF 导出失败" });
  }
});

/**
 * GET /api/v1/export-png/:id
 * 导出无损 PNG 图纸（高分辨率，放大清晰）
 */
router.get("/export-png/:id", async (req: Request, res: Response) => {
  try {
    const id = String(req.params.id);
    const image = getProcessedImage(id);
    if (!image) {
    ...
    const pngBuffer = await generatePatternPNG_v2(image, 24);
    ...
    res.status(500).json({ error: "PNG 导出失败" });
  }
});
